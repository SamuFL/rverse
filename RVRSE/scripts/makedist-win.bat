...
if %DEMO_ARG% == "demo" (
  echo Making RVRSE Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making RVRSE Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild RVRSE-app.vcxproj /p:configuration=release /p:platform=x64

echo Building x64 binaries...
REM add projects with /t to build VST2 and AAX
msbuild RVRSE.sln /t:RVRSE-app;RVRSE-vst3;RVRSE-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
msbuild RVRSE.sln /t:RVRSE-app;RVRSE-vst3;RVRSE-clap /p:configuration=release /p:platform=ARM64EC /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win-arm64ec.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\RVRSE.aaxplugin\Contents\Win32\RVRSE.aaxplugin --out .\build-win\aax\bin\RVRSE.aaxplugin\Contents\Win32\RVRSE.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\RVRSE.aaxplugin\Contents\x64\RVRSE.aaxplugin --out .\build-win\aax\bin\RVRSE.aaxplugin\Contents\x64\RVRSE.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\RVRSE.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\RVRSE.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "RVRSE Installer" ".\installer\RVRSE Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\RVRSE Installer.exe" ".\installer\RVRSE Demo Installer.exe"
  REM -del ".\installer\RVRSE Installer.exe"
  REM -)
